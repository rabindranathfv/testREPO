...
@Component({
  selector: 'app-customers-per-plants',
  templateUrl: './customers-per-plants.component.html',
  styleUrls: ['./customers-per-plants.component.scss']
})
export class CustomersPerPlantsComponent implements OnInit {
  ...
  public getPlantsByDevices() {
    this._dashboardService.getCardInfo('customers')
    .subscribe( (resp: any) => {
      this.datasCardBody = [...resp.object.object];
    });
    this._plantsService.getPlantsByResourceId(this.resource, this.id, 'customers')
        .subscribe( (resp) => {
          console.log(resp);

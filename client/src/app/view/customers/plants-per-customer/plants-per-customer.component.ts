...
@Component({
  selector: 'app-plants-per-customer',
  templateUrl: './plants-per-customer.component.html',
  styleUrls: ['./plants-per-customer.component.scss']
})
export class PlantsPerCustomerComponent implements OnInit {
  ...
  public getCustomersByDevices() {
    this._customersService.getCustomerByResourceId(this.resource, this.id, 'plants')
        .subscribe( (resp) => {
          console.log(resp);
  ...
  public getDataStructure() {
    this._dashboardService.getCardInfo('plants')
    .subscribe( (resp: any) => {
